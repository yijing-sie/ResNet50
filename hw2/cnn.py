...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            if i == 0:
                self.conv = Conv1D(num_input_channels, num_channels[i], kernel_sizes[i], strides[i], conv_weight_init_fn, bias_init_fn)
            else:
                self.conv = Conv1D(num_channels[i - 1], num_channels[i], kernel_sizes[i], strides[i], conv_weight_init_fn, bias_init_fn)
            self.out_width = ((self.input_width - kernel_sizes[i]) // strides[i]) + 1
            self.input_width = self.out_width
            self.convolutional_layers += [self.conv]
        self.linear_layer = Linear(self.out_width * num_channels[-1], num_linear_neurons, linear_weight_init_fn, bias_init_fn)
    ...
    def forward(self, x):
        ...
        self.output = x
        for i in range(self.nlayers):
            self.output = self.activations[i].forward(self.convolutional_layers[i](self.output))
            
        self.output = self.linear_layer.forward(self.flatten(self.output))
        return self.output
